...
async function loadAll() {
  ...
  const [activeRes, completedRes] = await Promise.all([
    supabaseClient
      ...
      .not('workflow_stage', 'in', '("activated","rejected")')
      .order('updated_at', { ascending: false }),

    supabaseClient
      .from('supplier_onboarding')
      .select('*, supplier:contacts(id, company_name, country)')
      .in('workflow_stage', ['activated', 'rejected'])
...
function updateKpis() {
  ...
  const pending = activeRows.filter(r => r.workflow_stage === 'pending_approval').length;
  ...
  const approved = completedRows.filter(r => r.workflow_stage === 'activated').length;
...
function renderStageBreakdown() {
  const stages = ['intake','screening','documents','compliance','recommendation','pending_approval'];
  const counts = {};
  ...
  el.innerHTML = stages.map(stage => {
    ...
    const badgeCls = OnboardingWorkflow.stageBadgeClass(stage);
    return `<div style="text-align:center;min-width:90px">
      ...
      <div style="font-size:10px;font-family:var(--font-display);font-weight:600;letter-spacing:0.05em;text-transform:uppercase;color:var(--color-text-muted)">${OnboardingWorkflow.stageLabel(stage)}</div>
    </div>`;
  }).join('');
}
...
function renderPipeline() {
  ...
  tbody.innerHTML = rows.map(r => {
    ...
    const isPendingApproval = r.workflow_stage === 'pending_approval';

    return `<tr ${isPendingApproval ? 'style="background:rgba(122,184,212,0.04)"' : ''}>
      <td>
      ...
      <td>
        ...
        ${isPendingApproval ? '<div style="font-size:10px;color:var(--color-accent);font-weight:600;margin-top:2px">Action needed</div>' : ''}
      </td>
      ...
      <td>
        <a href="detail.html?id=${esc(r.contact_id)}" class="btn btn-sm btn-ghost" style="border:1px solid var(--color-border);font-size:var(--text-xs);white-space:nowrap">
          ${isPendingApproval ? 'Review →' : 'Open →'}
        </a>
      </td>
    </tr>`;
  }).join('');
...
function renderCompleted() {
  ...
  tbody.innerHTML = completedRows.map(r => {
    const isActivated = r.workflow_stage === 'activated';
    const totalDays   = r.activated_at
      ? Math.floor((new Date(r.activated_at) - new Date(r.created_at)) / 86400000)
      : (r.updated_at ? daysSince(r.created_at) : null);
    ...
    return `<tr>
      ...
      <td>
        <span class="badge ${isActivated ? 'badge-success' : 'badge-danger'}">${isActivated ? 'Approved' : 'Rejected'}</span>
      ...
      <td style="font-size:var(--text-sm)">${fmtDate(r.activated_at || r.updated_at)}</td>
      <td style="font-size:var(--text-sm)">${totalDays !== null ? totalDays + ' days' : '—'}</td>
      <td>
